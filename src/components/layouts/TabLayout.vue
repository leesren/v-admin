<template>
  <global-layout ref="gRef" @dynamicRouterShow="dynamicRouterShow" @toggleCollapsed="handleToggleCollapsed">
    ...
    <div v-if="multipage" class="fixed-tabNav" :style="{ left: tabStyle }">
      <div class="mr50">
        <div class="tab-layout-tabs-nav">
          <a-tabs
    ...
    <div style="margin: 0 12px" :style="{ marginTop: multipage ? ' 64px' : '12px' }">
...
export default {
  ...
  computed: {
    ...
    tabStyle() {
      ...
      return (!this.sidebarCollapsed ? sidebar.openWidth : sidebar.closeWidth) + 'px'
    }
  },
  created() {
  ...
  methods: {
    handleToggleCollapsed(t) {
      this.sidebarCollapsed = t
    },
    changePage(key) {
      this.activePage = key
    },
